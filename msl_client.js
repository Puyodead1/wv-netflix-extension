...
async function getViewableId(viewableIdPath) {
  ...
  console.log("viewable id response:");
  console.log(apiJson);
...
async function performKeyExchange() {
  ...
  var handshakeResp = await fetch(manifestUrl, {
    body: request,
    method: "POST",
  });
...
async function decryptMslResponse(data) {
  ...
  for (i = 0; i < payloadChunks.length; i++) {
    ...
    var plaintext = JSON.parse(
      arrayBufferToString(aesjs.padding.pkcs7.strip(aesCbc.decrypt(ciphertext)))
    );
...
async function getManifest(esn = defaultEsn, id) {
  ...
  var manifestRequestData = {
    ...
    esn: defaultEsn,
    languages: [localeId],
    uiVersion: "shakti-v6a5172d2",
    clientVersion: "6.0026.529.051",
    params: {
      type: "standard",
  ...
  var encryptedManifestRequest = await generateMslRequestData(
    manifestRequestData
  );
...
async function getLicense(challenge, sessionId) {
  ...
  var encryptedLicenseRequest = await generateMslRequestData(
    licenseRequestData
  );
  var licenseResp = await fetch(licenseUrl, {
